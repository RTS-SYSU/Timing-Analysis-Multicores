...
This is a fork of LLVM-TA+ built by University of Saarland, you can find their version at [here](https://gitlab.cs.uni-saarland.de/reineke/LLVM-TA+)

# LLVM-TA+

This Repository contains LLVM-TA+ a static timing analysis tool based on the LLVM compiler ecosystem.
...
```
docker build -t LLVM-TA+docker:latest - < .devcontainer/Dockerfile
```
...
```
docker run -i -d \
	-v {$path_to_this_repository}:/workspaces/LLVM-TA+:rw \
	-v {$path_to_build_bir}:/workspaces/LLVM-TA+/build:rw \
	--name $name_of_instance LLVM-TA+docker:latest
```
...
1.) Config the project with the ./config.sh script. Be aware that llvm and LLVM-TA+ requires a lot or RAM, should your System have only 16GB of RAM choose the "lowRes" configuration.

```
Script to configure llvm, clang and LLVM-TA+:
...
3.) Build llvm, clang and LLVM-TA+ with ninja. This is necessary at least once, so that the llvm and LLVM-TA+ toolchain are of the same version.
...
For later rebuilds it is enough to just build LLVM-TA+.

```
cd build
ninja -j [#CPUs] LLVM-TA+
```
<!-- 
...
This section showcases how to use LLVM-TA+ on a simple test case of a nested loop example. Make sure LLVM-TA+ is compiled and in your $PATH variable.
...
2.) Run LLVM-TA+ on the benchmark using the script
...
The license will not be found, if its not named gurobi.lic! Otherwise LLVM-TA+ will fallback to using LPsolve.
