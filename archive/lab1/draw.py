...
if __name__ == "__main__":
    # 定义要执行的命令列表
    commands = [
        ["../post_run.py", "-s", "0712outputzw", "-t", "0712outputzw", "-m", "zhangw"],
        ["../post_run.py", "-s", "0711output", "-t", "0711output", "-m", "our"]
    ]
    ...
    inter_latency_df = pd.read_csv(os.path.join(base_dir, '0711output', 'wceet.csv'), index_col=0)
    intra_latency_df = pd.read_csv(os.path.join(base_dir, '0711output', 'intra_wcet.csv'), index_col=0)

    inter_latency_df_zw = pd.read_csv(os.path.join(base_dir, '0712outputzw', 'wceet.csv'), index_col=0)
    intra_latency_df_zw = pd.read_csv(os.path.join(base_dir, '0712outputzw', 'intra_wcet.csv'), index_col=0)
