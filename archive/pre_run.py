...
def collect_c_h_files(src_folder):
    """收集文件夹下的所有 .c 和 .h 文件的完整路径和相对路径"""
    c_and_h_files = []
    for root, _, files in os.walk(src_folder):
        for file in files:
            if file.endswith('.c') or file.endswith('.h'):
                full_path = os.path.join(root, file)
                # 计算相对于src_folder的相对路径
                rel_path = os.path.relpath(full_path, src_folder)
                c_and_h_files.append((full_path, rel_path))
    return c_and_h_files
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="两两组合子文件夹中的 .c/.h 文件并输出到组合目录中")
