...
void sha_transform(struct SHA_INFO *sha_info) {
  ...
  _Pragma("loopbound min 16 max 16") for (i = 0; i < 16; ++i) W[i] =
      sha_info->data[i];
  _Pragma("loopbound min 64 max 64") for (i = 16; i < 80; ++i) W[i] =
  ...
  _Pragma("loopbound min 20 max 20") for (i = 0; i < 20; ++i) { FUNC(1, i); }
  _Pragma("loopbound min 20 max 20") for (i = 20; i < 40; ++i) { FUNC(2, i); }
  _Pragma("loopbound min 20 max 20") for (i = 40; i < 60; ++i) { FUNC(3, i); }
  _Pragma("loopbound min 20 max 20") for (i = 60; i < 80; ++i) { FUNC(4, i); }
  sha_info->digest[0] += A;
  sha_info->digest[1] += B;
  sha_info->digest[2] += C;
  sha_info->digest[3] += D;
...
void sha_byte_reverse(LONG *buffer, int count) {
  ...
  count /= sizeof(LONG);
  cp = (BYTE *)buffer;
  _Pragma("loopbound min 16 max 16") for (i = 0; i < count; ++i) {
    ct[0] = cp[0];
    ct[1] = cp[1];
    ct[2] = cp[2];
    ct[3] = cp[3];
...
size_t sha_fread(void *ptr, size_t size, size_t count,
  ...
  _Pragma("loopbound min 0 max 8192") while (i < stream->cur_pos +
                                                     number_of_chars_to_read)(
...
void sha_update(struct SHA_INFO *sha_info, BYTE *buffer, int count) {
  ...
  _Pragma("loopbound min 8 max 128") while (count >= SHA_BLOCKSIZE) {
...
void sha_stream(struct SHA_INFO *sha_info, struct SHA_MY_FILE *fin) {
  ...
  _Pragma("loopbound min 5 max 5") while (
      (i = sha_fread(data, 1, BLOCK_SIZE, fin)) > 0)
